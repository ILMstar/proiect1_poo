...
void place_shop(lista& lista_curenta) {
    ...
    for (int j = 0; j < 8; j++)
        for (int i = 0; i < 4; i++) {
            std::cout << "The trader has arrived and now is setting up the shop! Loading " << loading_anim[i];
            Sleep(50);
            std::system("cls");
        }
    lista_curenta.item_select(10); // selecteaza 10 iteme
    lista_curenta.show_selected_itms(10);
    int item_ids[4]; // iau id-ul itemelor
    for (int i = 0; i < 4; i++) {
        item_ids[i] = lista_curenta.get_display(i);
    }
